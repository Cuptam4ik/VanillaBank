...
// --- ИСПРАВЛЕНО: ЕДИНЫЙ МАРШРУТ ДЛЯ ПОИСКА ПОЛЬЗОВАТЕЛЕЙ ---
// Этот один блок заменяет два предыдущих (дублирующихся)
app.get('/api/users/search', userAuth, async (req, res) => {
    const { nickname, cardNumber } = req.query; // Принимаем и ник, и карту

    // Проверяем, что есть хотя бы один из параметров и он достаточно длинный
    ...
    try {
        ...
        if (nickname) {
            ...
            // Фильтруем пользователей, чья карта НАЧИНАЕТСЯ с введенных цифр
            matchingUsers = allUsers
                .filter(user => String(user.cardNumber).startsWith(cardNumber))
                .slice(0, 5); // Берем первые 5 совпадений
        }
